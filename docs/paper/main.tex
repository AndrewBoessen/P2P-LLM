...
$$w_i = \alpha \cdot c_i + (1 - \alpha) \cdot p_i$$
Howver, this assumes that the weight $\alpha$ is known a priori. In general, there are many equivilent optimal solutions in a MO optimzation problem.
...
\begin{enumerate}
	\item $\forall i, f_i(x_1) \leq f_i(x_2)$
...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[
				...
				height=8cm,
				xlabel={$p_i$},
				ylabel={Expected Revenue $R(p_i, c_i)$},
				domain=0:12,
				samples=200,
				legend pos=north east,
				grid=major,
				...
				title={Expected Revenue Function $R(p_i, c_i)$}
			]
